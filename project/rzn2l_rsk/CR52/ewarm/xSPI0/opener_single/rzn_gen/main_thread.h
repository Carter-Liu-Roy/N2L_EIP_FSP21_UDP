...
/** UART on SCI Instance. */
            extern const uart_instance_t      g_uart_safety;

            /** Access the UART instance using these structures when calling API functions directly (::p_api is not used). */
            extern sci_uart_instance_ctrl_t     g_uart_safety_ctrl;
            extern const uart_cfg_t g_uart_safety_cfg;
            extern const sci_uart_extended_cfg_t g_uart_safety_cfg_extend;

            #ifndef cip_safety_sci_callback
            void cip_safety_sci_callback(uart_callback_args_t * p_args);
            #endif

            #define FSP_NOT_DEFINED (1)
            #if (FSP_NOT_DEFINED == FSP_NOT_DEFINED)
                #define g_uart_safety_P_TRANSFER_TX (NULL)
            #else
                #define g_uart_safety_P_TRANSFER_TX (&FSP_NOT_DEFINED)
            #endif
            #if (FSP_NOT_DEFINED == FSP_NOT_DEFINED)
                #define g_uart_safety_P_TRANSFER_RX (NULL)
            #else
                #define g_uart_safety_P_TRANSFER_RX (&FSP_NOT_DEFINED)
            #endif
