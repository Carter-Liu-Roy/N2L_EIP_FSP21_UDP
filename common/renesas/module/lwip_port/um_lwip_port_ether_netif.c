...
void _ether_netif_callback( ether_netif_callback_args_t * p_args )
{
	...
	case ETHER_NETIF_CALLBACK_EVENT_ANY_PORT_LINK_STATUS_CHANGED:
        ...
        /** If port 1 link up */
	    if( ((p_ctrl->port_link_status & ETHER_NETIF_PORT_BIT(0)) == 0) &&
	        ((p_args->port_link_status & ETHER_NETIF_PORT_BIT(0)) == ETHER_NETIF_PORT_BIT(0)) )
	    {
            /** Issue callback event with link up.*/
	        usr_err = um_lwip_port_task_launcher_request(p_ctrl->p_launcher_ctrl, LWIP_PORT_LAUNCHER_EVENT_PORT1_LINK_UP, NULL);
	        if ( !(USR_SUCCESS == usr_err || USR_ERR_NOT_ENABLED == usr_err) )
	        {
	            /** TODO: Implement error handling in callback context. */
	        }
        ...
        /** If port 1 link down, */
	    else if( ((p_ctrl->port_link_status & ETHER_NETIF_PORT_BIT(0)) == ETHER_NETIF_PORT_BIT(0)) &&
	             ((p_args->port_link_status & ETHER_NETIF_PORT_BIT(0)) == 0) )
        {
            /** Issue callback event with link down.*/
            usr_err = um_lwip_port_task_launcher_request(p_ctrl->p_launcher_ctrl, LWIP_PORT_LAUNCHER_EVENT_PORT1_LINK_DOWN, NULL);
            if ( !(USR_SUCCESS == usr_err || USR_ERR_NOT_ENABLED == usr_err) )
            {
                /** TODO: Implement error handling in callback context. */
            }
        ...
        /** If port 2 link up */
	    if( ((p_ctrl->port_link_status & ETHER_NETIF_PORT_BIT(1)) == 0) &&
	        ((p_args->port_link_status & ETHER_NETIF_PORT_BIT(1)) == ETHER_NETIF_PORT_BIT(1)) )
	    {
            /** Issue callback event with link up.*/
	        usr_err = um_lwip_port_task_launcher_request(p_ctrl->p_launcher_ctrl, LWIP_PORT_LAUNCHER_EVENT_PORT2_LINK_UP, NULL);
	        if ( !(USR_SUCCESS == usr_err || USR_ERR_NOT_ENABLED == usr_err) )
	        {
	            /** TODO: Implement error handling in callback context. */
	        }
        ...
        /** If port 2 link down, */
	    else if( ((p_ctrl->port_link_status & ETHER_NETIF_PORT_BIT(1)) == ETHER_NETIF_PORT_BIT(1)) &&
	             ((p_args->port_link_status & ETHER_NETIF_PORT_BIT(1)) == 0) )
        {
            /** Issue callback event with link down.*/
            usr_err = um_lwip_port_task_launcher_request(p_ctrl->p_launcher_ctrl, LWIP_PORT_LAUNCHER_EVENT_PORT2_LINK_DOWN, NULL);
            if ( !(USR_SUCCESS == usr_err || USR_ERR_NOT_ENABLED == usr_err) )
            {
                /** TODO: Implement error handling in callback context. */
            }
