...
#if LWIP_LLDP
static err_t _lldp_netif_add_callback(struct netif *p_netif);
#endif

/** Callback functions called by lwIP network interface. */
static void  _status_changed(struct netif *netif);
static void  _acd_changed(struct netif *netif);
								   ...
								   lwip_port_launcher_ctrl_t * const p_launcher_ctrl )
{
...
#if LWIP_LLDP
    lwip_err = LWIP_LLDP_agent_init(&p_ctrl->netif);
    ...
    lwip_err = netifapi_netif_add(
            ...
            (LWIP_PORT_DHCP_ENABLE == p_ctrl->p_netif_cfg->dhcp) ? NULL : (ip4_addr_t*) &p_cfg->gateway_address,
            p_ctrl, // Pass instance control to "netif->state" controlled in TCP/IP task context.
            (netif_init_fn) _lldp_netif_add_callback, // Pass callback function for setting network interface.
            (netif_input_fn) tcpip_input);
    USR_ERROR_RETURN( ERR_OK == lwip_err, USR_ERR_NOT_INITIALIZED );

    lwip_err = LWIP_LLDP_agent_init(&lldp_netif);
...
/*******************************************************************************************************************//**
 * @brief Get the network interface infomation.
 *
 * @param[in] p_ctrl                    Pointer to the controller
 * @param[out] p_netif_info             Pointer to the netif infomation to be written.
 *
 * @retval USR_SUCCESS                  Process has been done successfully.
 **********************************************************************************************************************/
usr_err_t um_lwip_port_netif_get_netif_info( lwip_port_netif_ctrl_t * const p_ctrl, lwip_port_netif_info_t * p_netif_info )
{
    struct acd *acd;

    ACD_FOREACH(acd, p_ctrl->netif.acd_list)
    {
        memset(p_netif_info, 0, sizeof(lwip_port_netif_info_t));
        p_netif_info->acd_status = acd->acd_status;
        p_netif_info->acd_fault = acd->acd_fault;
        if ((acd->state == ACD_STATE_FAULT) || (acd->state == ACD_STATE_DEFEND)) {
            p_netif_info->acd_nvdata_flag = 1;
            p_netif_info->acd_activity = acd->acd_activity;
            memcpy(p_netif_info->remote_mac, acd->remote_mac, sizeof(p_netif_info->remote_mac));
            memcpy(p_netif_info->arp_pdu, acd->arp_pdu, sizeof(p_netif_info->arp_pdu));
        }
    }
...
/*******************************************************************************************************************//**
 * @brief Get the network interface acd enable flag.
 *
 * @param[in] p_ctrl                    Pointer to the controller
 * @param[in] acd_enable                New acd enable flag of netif.
 *
 * @retval USR_SUCCESS                  Process has been done successfully.
 *********************************************************************************************************************/
usr_err_t um_lwip_port_netif_set_netif_acd( lwip_port_netif_ctrl_t * const p_ctrl, uint8_t acd_enable )
{
    /** Status */
    ...
    {
        /** Tells ACD to enable. */
        lwip_err = netifapi_acd_select_enable(&(p_ctrl->netif));
        USR_ERROR_RETURN( ERR_OK == lwip_err, USR_ERR_ABORTED );
    }
    else
    {
        /** Tells ACD to disable. */
        lwip_err = netifapi_acd_select_disable(&(p_ctrl->netif));
        USR_ERROR_RETURN( ERR_OK == lwip_err, USR_ERR_ABORTED );
    }
...
#if LWIP_LLDP
	if ( LWIP_LLDP_receive_check(p_frame_packet) == 0 )
	{
		return USR_SUCCESS;
	}
#endif
...
usr_err_t um_lwip_port_netif_set_up( lwip_port_netif_ctrl_t * const p_ctrl )
{
    ...
    {
        /** Start ACD for Static IP Address */
        lwip_err = netifapi_static_ip_acd_start(&(p_ctrl->netif));
        USR_ERROR_RETURN( ERR_OK == lwip_err, USR_ERR_ABORTED );
    }
...
/*******************************************************************************************************************//**
 * @brief Set link up lwIP network interface.
 *
 * @param[in] p_ctrl					Pointer to the controller
 *
 * @retval USR_SUCCESS					Process has been done successfully.
 * @retval USR_ERR_NOT_INITIALIZED		Initialization has been failed.
 **********************************************************************************************************************/
usr_err_t um_lwip_port_netif_set_link_up( lwip_port_netif_ctrl_t * const p_ctrl )
{
    /** Status */
    err_t lwip_err = ERR_OK;

    /** Tell ACD that the physical link is up. */
    lwip_err = netifapi_acd_link_up(&(p_ctrl->netif));
    USR_ERROR_RETURN( ERR_OK == lwip_err, USR_ERR_ABORTED );
...
/*******************************************************************************************************************//**
 * @brief Set link down lwIP network interface.
 *
 * @param[in] p_ctrl					Pointer to the controller
 *
 * @retval USR_SUCCESS					Process has been done successfully.
 * @retval USR_ERR_NOT_INITIALIZED		Initialization has been failed.
 **********************************************************************************************************************/
usr_err_t um_lwip_port_netif_set_link_down( lwip_port_netif_ctrl_t * const p_ctrl )
{
    /** Status */
    err_t lwip_err = ERR_OK;

    /** Tell ACD that the physical link is down. */
    lwip_err = netifapi_acd_link_down(&(p_ctrl->netif));
    USR_ERROR_RETURN( ERR_OK == lwip_err, USR_ERR_ABORTED );
...
static err_t _netif_add_callback(struct netif *p_netif)
{
...
#if LWIP_LLDP
    memset((void *)&p_netif->name[0U], 0, sizeof(p_netif->name));
    p_netif->name[0U] = 'P';
    p_netif->name[1U] = 'O';
    p_netif->name[2U] = 'R';
    p_netif->name[3U] = 'T';
    p_netif->name[4U] = ' ';
    p_netif->name[5U] = '1';
#else /** RE for Renesas Electronics */
    p_netif->name[0U] = 'R';
    p_netif->name[1U] = 'E';
#endif
    ...
    /** Set the callback when the acd has been changed */
    p_netif->acd_callback = (netif_status_callback_fn) _acd_changed;
...
/*******************************************************************************************************************//**
 * Callback function called by lwIP when an ARP conflict detection.
 *
 * @param[in] p_netif   Pointer to lwIP network interface structure
 **********************************************************************************************************************/
static void _acd_changed(struct netif *p_netif)
{
    /** Resolve context pointer type */
    lwip_port_netif_ctrl_t * p_ctrl = (lwip_port_netif_ctrl_t * ) p_netif->state;

    /** Request the link up event for launch callback when an ARP conflict detection */
    (void) um_lwip_port_task_launcher_request(p_ctrl->p_launcher_ctrl, LWIP_PORT_LAUNCHER_EVENT_ACD, p_ctrl);
}
