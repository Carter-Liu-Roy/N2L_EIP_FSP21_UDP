...
/*******************************************************************************************************************//**
 * @defgroup LWIP_PORT_API TCP/IP lwIP Port
 * @ingroup RENESAS_INTERFACES
 * @brief Interface for LWIP_PORT functions.
 *
 * @section LWIP_PORT_API_SUMMARY Summary
 * The LWIP_PORT interface provides the network interface for utilizing TCP/IP communication.
...
typedef enum e_lwip_port_acd
{
    LWIP_PORT_ACD_DISABLE,         ///< Event when ACD is disabled
    LWIP_PORT_ACD_ENABLE,          ///< Event when ACD is enabled
} lwip_port_acd_t;

typedef enum e_lwip_port_lldp_admin_status
{
    LWIP_PORT_LLDP_enabledTxOnly = 1,
    LWIP_PORT_LLDP_enabledRxOnly = 2,
    LWIP_PORT_LLDP_enabledRxTx = 3,
    LWIP_PORT_LLDP_disabled = 4
} lwip_port_lldp_admin_status_t;
 ...
 */
typedef struct st_lwip_port_netif_info
{
    uint8_t acd_status;       ///< TCP/IP Interface Object - Attribute 1 -  Status : AcdStatus
    uint8_t acd_fault;        ///< TCP/IP Interface Object - Attribute 1 -  Status : AcdFault
    uint8_t acd_nvdata_flag;  ///< Flag indicating that TCP/IP configurable nvdata should be stored
    uint8_t acd_activity;     ///< TCP/IP Interface Object - Attribute 11 - LastConflictDetected : AcdActivity
    uint8_t remote_mac[6];    ///< TCP/IP Interface Object - Attribute 11 - LastConflictDetected : RemoteMAC
    uint8_t arp_pdu[28];      ///< TCP/IP Interface Object - Attribute 11 - LastConflictDetected : ArpPdu
} lwip_port_netif_info_t;
...
typedef enum e_lwip_port_callback_event
{
    ...
    LWIP_PORT_CALLBACK_EVENT_ACD,	     ///< Event when an ARP conflict detection.
...
/** LWIP_PORT control block. Allocate an instance specific control block to pass into the LWIP_PORT API calls.
 * @par Implemented as
...
/** LWIP_PORT API structure. General net functions implemented at the HAL layer follow this API. */
typedef struct st_lwip_port_api
{
    ...
    /** Get the netif information of lwIP network interface.
     * @par Implemented as
     * - @ref UM_LWIP_PORT_NetifInfoGet()
     *
     * @param[in]  p_ctrl         Pointer to control structure.
     * @param[out]  p_netif_info  Pointer to netif information.
     * @return
     */
    usr_err_t (* netifInfoGet)( lwip_port_ctrl_t * const p_ctrl, lwip_port_netif_info_t * p_netif_info );


    /** Get the netif acd enable flag of lwIP network interface.
     * @par Implemented as
     * - @ref UM_LWIP_PORT_NetifAcdSet()
     *
     * @param[in]  p_ctrl         Pointer to control structure.
     * @param[in]  acd_enable     New acd enable flag of netif.
     * @return
     */
    usr_err_t (* netifAcdSet)( lwip_port_ctrl_t * const p_ctrl, uint8_t acd_enable );
