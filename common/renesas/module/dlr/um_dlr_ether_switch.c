...
static usr_err_t _get_last_beacon_receive_port( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_port_bit_t * const p_beacon_info );
...
usr_err_t um_dlr_ether_switch_open( dlr_ether_switch_ctrl_t * const p_ctrl,
									...
									ether_switch_instance_t const * const p_ether_switch_instance )
{
	/** Error codes */
	fsp_err_t fsp_err;
...
usr_err_t um_dlr_ether_switch_init_dlr( dlr_ether_switch_ctrl_t * const p_ctrl )
{
	...
	USR_LOCK_SECTION_START(p_ctrl->p_mutex_handle);
	{
		...
		usr_err = um_dlr_ether_netif_get_local_mac_address(p_ctrl->p_ether_netif_ctrl, (uint8_t *) &p_ctrl->dlr_host_mac_addr );
		USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == (fsp_err_t) usr_err,  USR_ERR_NOT_INITIALIZED );
		...
		/** Initialize DLR */
		fsp_err = R_ETHSW_DlrInitSet( p_ctrl->p_target->p_ctrl, &p_ctrl->dlr_init_cfg );
		USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err,  USR_ERR_NOT_INITIALIZED );

		/** Enable DLR extension module. */
		fsp_err = R_ETHSW_DlrEnableSet( p_ctrl->p_target->p_ctrl );
		USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err,  USR_ERR_NOT_INITIALIZED );
	}
	USR_LOCK_SECTION_END(p_ctrl->p_mutex_handle);

    /** Disable Beacon frame forwarding */
	ethsw_mac_table_entry_addr_t mac_entry_addr = {0};
	ethsw_mac_table_entry_info_t mac_entry_info = {0};
    uint8_t dlr_mac[ETHSW_MAC_ADDR_LENGTH] = {0x01, 0x21, 0x6c, 0x00, 0x00, 0x01}; /* Beacon frame MAC address */
    mac_entry_addr.mac_addr = (ethsw_mac_addr_t *) &dlr_mac;
    dlr_mac[5]          = 0x01;
    mac_entry_info.port_mask.mask = ETHSW_PORT_BIT(2); /* This means transmission from both port0 and port1 is disabled.  */
    fsp_err = R_ETHSW_MacTableSet(p_ctrl->p_target->p_ctrl, &mac_entry_addr, &mac_entry_info);
...
usr_err_t um_dlr_ether_switch_shutdown_dlr( dlr_ether_switch_ctrl_t * const p_ctrl )
{
	...
	{
		/** Shutdown DLR */
		fsp_err = R_ETHSW_DlrDisableSet( p_ctrl->p_target->p_ctrl );
...
usr_err_t um_dlr_ether_switch_flush_unicast_mac_table( dlr_ether_switch_ctrl_t * const p_ctrl )
{
	...
	ethsw_mac_table_clear_mode_t mactab_clr = ETHSW_MAC_TABLE_CLEAR_DYNAMIC;

	/** Enter lock statement. */
	USR_LOCK_SECTION_START(p_ctrl->p_mutex_handle);
	{
		/** Flush unicast MAC table */
		fsp_err = R_ETHSW_MacTableClear( p_ctrl->p_target->p_ctrl, &mactab_clr );
		USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );
	}
	USR_LOCK_SECTION_END(p_ctrl->p_mutex_handle);
											  ...
											  dlr_node_state_t * p_node_state )
{
	...
	{
		/** Get node state */
		usr_err = _get_node_state( p_ctrl, p_node_state );
 ...
 * @retval USR_ERR_ABORTED			Process has failed.
 **********************************************************************************************************************/
usr_err_t um_dlr_ether_switch_get_last_beacon_receive_port( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_port_bit_t * const p_last_beacon_receive_port )
{
	/** Error codes */
	usr_err_t usr_err;
 ...
 * @retval USR_ERR_ABORTED			Process has failed.
 **********************************************************************************************************************/
usr_err_t um_dlr_ether_switch_stop_forwarding_frames( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_port_bit_t port )
{
	/** Error codes */
	fsp_err_t fsp_err;
	...
	USR_LOCK_SECTION_START(p_ctrl->p_mutex_handle);
	{
		...
		{
			fsp_err = R_ETHSW_PortForwardDel( p_ctrl->p_target->p_ctrl, DLR_PORT0_NUM );
			USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );
		}
		...
		{
			fsp_err = R_ETHSW_PortForwardDel( p_ctrl->p_target->p_ctrl, DLR_PORT1_NUM );
			USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );
		}
	}
	USR_LOCK_SECTION_END(p_ctrl->p_mutex_handle);
 ...
 * @retval USR_ERR_ABORTED			Process has failed.
 **********************************************************************************************************************/
usr_err_t um_dlr_ether_switch_start_forwarding_frames( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_port_bit_t port )
{
	/** Error codes */
	fsp_err_t fsp_err;
	...
	USR_LOCK_SECTION_START(p_ctrl->p_mutex_handle);
	{
		...
		if( DLR_PORT0_BIT == port || DLR_PORT_BOTH == port )
		{
			fsp_err = R_ETHSW_PortForwardAdd( p_ctrl->p_target->p_ctrl, DLR_PORT0_NUM );
			USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );
		}
		...
		if( DLR_PORT1_BIT == port || DLR_PORT_BOTH == port )
		{
			fsp_err = R_ETHSW_PortForwardAdd( p_ctrl->p_target->p_ctrl, DLR_PORT1_NUM );
			USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );
		}
	}
	USR_LOCK_SECTION_END(p_ctrl->p_mutex_handle);
 ...
 * @retval USR_ERR_ABORTED			Process has failed.
 **********************************************************************************************************************/
usr_err_t um_dlr_ether_switch_get_link_state( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_port_bit_t * const p_dlr_port )
{
	/** Error codes */
	fsp_err_t fsp_err;

	/** Temporary variables */
	ethsw_link_status_t state_link0;
	ethsw_link_status_t state_link1;

	/** Enter lock statement. */
	USR_LOCK_SECTION_START(p_ctrl->p_mutex_handle);
	{
		fsp_err = R_ETHSW_LinkStateGet( p_ctrl->p_target->p_ctrl, DLR_PORT0_NUM, &state_link0 );
		USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );
		fsp_err = R_ETHSW_LinkStateGet( p_ctrl->p_target->p_ctrl, DLR_PORT0_NUM, &state_link1 );
		USR_LOCK_ERROR_RETURN( p_ctrl->p_mutex_handle, FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );

		*p_dlr_port = (dlr_port_bit_t) ( (state_link1 << 1) | (state_link0 << 0) );
	}
	USR_LOCK_SECTION_END(p_ctrl->p_mutex_handle);
...
static void _dlr_callback_code( ethsw_dlr_event_t event, uint32_t port )
{
	...
	dlr_event.event = (ethsw_dlr_event_extend_t) event;
	switch (port)
	{
...
static usr_err_t _get_node_state( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_node_state_t * p_node_state )
{
	...
	/** Get node state */
	fsp_err = R_ETHSW_DlrNodeStateGet( p_ctrl->p_target->p_ctrl, (ethsw_dlr_node_state_t *) &node_state );
...
static usr_err_t _get_beacon_info( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_rx_beacon_t * const p_beacon_info )
{
	...
	fsp_err = R_ETHSW_DlrSvMacGet( p_ctrl->p_target->p_ctrl, &p_beacon_info->source_mac_address );
	USR_ERROR_RETURN( FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );

    /** Get store supervisor IPADDR address stored in DLR_SUPR_IPADR register */
	fsp_err = R_ETHSW_DlrSvIpGet( p_ctrl->p_target->p_ctrl, &p_beacon_info->source_ip_address );
	USR_ERROR_RETURN( FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );

    /** Get precedence value of the supervisor stored in DLR_SUPR_MAC register. */
	fsp_err = R_ETHSW_DlrSvPriorityGet( p_ctrl->p_target->p_ctrl, (uint8_t *) &precedence );
	USR_ERROR_RETURN( FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );

    /** Get VLAN tag control filed in Beacon frame stored DLR_STATE_VLAN register. */
	fsp_err = R_ETHSW_DlrVlanGet( p_ctrl->p_target->p_ctrl, (uint16_t *) &vlan_id );
	USR_ERROR_RETURN( FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );

    /** Get Ring State field in Beacon frame stored DLR_STATE_VLAN register. */
	fsp_err = R_ETHSW_DlrNodeStateGet( p_ctrl->p_target->p_ctrl, (ethsw_dlr_node_state_t *) &ring_state );
	USR_ERROR_RETURN( FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );
...
static usr_err_t _get_last_beacon_receive_port( dlr_ether_switch_ctrl_t * const p_ctrl, dlr_port_bit_t * const p_last_beacon_receive_port )
{
	/** Error codes */
	fsp_err_t fsp_err;
	...
	fsp_err = R_ETHSW_DlrBeaconStateGet( p_ctrl->p_target->p_ctrl, DLR_PORT0_NUM, (ethsw_dlr_beacon_state_t *) &port0_last_beacon_received );
	USR_ERROR_RETURN( FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );

    /** Get LastBcnRcvPort[1] */
	fsp_err = R_ETHSW_DlrBeaconStateGet( p_ctrl->p_target->p_ctrl, DLR_PORT1_NUM, (ethsw_dlr_beacon_state_t *) &port1_last_beacon_received );
	USR_ERROR_RETURN( FSP_SUCCESS == fsp_err, USR_ERR_ABORTED );

	/** Store values needing cast. */
	*p_last_beacon_receive_port = (dlr_port_bit_t) ( (port1_last_beacon_received << 1) |
