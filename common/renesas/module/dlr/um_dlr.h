...
typedef struct st_dlr_beacon_frame_info
{
    ...
    dlr_port_bit_t last_beacon_receive_port;   ///< Not filed: Last beacon receive port
} dlr_rx_beacon_t;

/** DLR event */
typedef struct st_dlr_event_arg
{
    ethsw_dlr_event_extend_t event;
	dlr_port_bit_t           event_port;
...
typedef struct st_dlr_state_machine_ctrl
{
    ...
    dlr_port_bit_t link_active_port;
    dlr_port_bit_t neighbor_active_port;
...
typedef struct st_dlr_ether_switch_ctrl
{
	...
	ether_switch_instance_t const * p_target;
