...
#define OPENER_PORT_NET_STATE_NUM                 (7)       ///< The number of network state
#else
...
#if defined(OPENER_CIP_SAFETY)
    OPENER_PORT_NETWORK_STATE_WAIT_FOR_TUNID,             /* Waiting for TUNID when Proposed TUNID recieved */                                ///< OPENER_PORT_NETWORK_STATE_WAIT_FOR_TUNID
#endif
    OPENER_PORT_NETWORK_STATE_SELF_TEST,                                                                                                      ///< OPENER_PORT_NETWORK_STATE_SELF_TEST
} opener_port_network_state_t ;

/**
 * @brief Enumerator for state to store nvdata
 */
typedef enum e_opener_port_nvdata_state
{
    OPENER_PORT_NVDATA_STATE_NON,     ///< State where there is no need to store nvdata
    OPENER_PORT_NVDATA_STATE_FOR_TCP, ///< State where it is necessary to store TCP/IP settable nvdata
} opener_port_nvdata_state_t;
...
typedef struct st_opener_port_lwip_port_ctrl
{
	...
	opener_port_nvdata_state_t      nvdata_state;               ///< State to store nvdata
    ...
    void * p_lldpmanagement_cip_instance;                       ///< Pointer to CIP LLDP Management object
