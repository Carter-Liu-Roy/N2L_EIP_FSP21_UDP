...
static usr_err_t _update_ethernetip_module_state ( opener_port_netif_ctrl_t * p_ctrl );
...
static void _task_code (void * pvParameter)
{
    ...
    opener_port_netif_ctrl_t * const p_ctrl = (opener_port_netif_ctrl_t *) pvParameter;
    ...
    while(true)
    {
        ...
        /** Update module state. */
        usr_err = _update_ethernetip_module_state( p_ctrl );
        if( USR_SUCCESS != usr_err )
        {
            /** TODO: Error handling in task. */
        }
...
static usr_err_t _update_ethernetip_network_state ( opener_port_netif_ctrl_t * p_ctrl )
{
    ...
    /** Resolve task parameter */
    opener_port_indicator_ctrl_t * const p_indicator_ctrl = (opener_port_indicator_ctrl_t * const) p_ctrl->p_indicator_ctrl;
    ...
    {
        usr_err = um_opener_port_lwip_port_netif_has_ip_duplicate( p_ctrl->p_lwip_port_ctrl );
        if ( USR_SUCCESS == usr_err )
        {
            /** Duplicate IP address. */
            new_network_state = OPENER_PORT_NETWORK_STATE_DUPLICATE_IP;
        }
        else
        {
        /** No IP address. */
    ...
    usr_err = um_opener_port_lwip_port_netif_rcv_proposed_tunid( p_ctrl->p_lwip_port_ctrl );
    if ( USR_SUCCESS == usr_err )
    {
        /** Waiting for TUNID when Proposed TUNID recieved */
        new_network_state = OPENER_PORT_NETWORK_STATE_WAIT_FOR_TUNID;
    }
#endif
    ...
    /** Check if lwIP network interface need to store TCP/IP settable nvdata. */
    usr_err = um_opener_port_lwip_port_state_need_store_nvdata_tcp( p_ctrl->p_lwip_port_ctrl );
    if ( USR_SUCCESS == usr_err )
    {
        usr_err = um_opener_port_callback_request( p_indicator_ctrl->p_callback_ctrl, OPENER_PORT_CALLBACK_STORE_NVDATA_FOR_TCP, off );
        USR_ERROR_RETURN( USR_SUCCESS == usr_err, USR_ERR_ABORTED );
    }
...
static usr_err_t _update_ethernetip_module_state ( opener_port_netif_ctrl_t * p_ctrl )
{
    ...
    /** New module state. */
    opener_port_module_state_t cur_module_state;
    opener_port_module_state_t new_module_state;

    /** Get current module state */
    usr_err = um_opener_port_task_indicator_get_module_state( p_ctrl->p_indicator_ctrl, &cur_module_state );

    /** Check if lwIP port module interface state. */
    usr_err = um_opener_port_lwip_port_netif_has_ip_duplicate( p_ctrl->p_lwip_port_ctrl );
    if ( USR_SUCCESS == usr_err )
    {
        /** Major Recoverable Fault. */
        new_module_state = OPENER_PORT_MODULE_STATE_MAJOR_RECOVERABLE_FAULT;
    }
    else
    {
        if( cur_module_state == OPENER_PORT_MODULE_STATE_MAJOR_RECOVERABLE_FAULT )
        {
            /** Standby. */
            new_module_state = OPENER_PORT_MODULE_STATE_STANDBY;
        }
        else
        {
            /** Operational. */
            new_module_state = OPENER_PORT_MODULE_STATE_DEVICE_OPERATIONAL;
        }
    }

    /** Update module state */
    if( cur_module_state != new_module_state )
    {
        usr_err = um_opener_port_task_indicator_set_module_state( p_ctrl->p_indicator_ctrl, new_module_state );
        USR_ERROR_RETURN( USR_SUCCESS == usr_err, USR_ERR_ABORTED );
    }
