...
/**
 * For CIP LLDP Management object
 */
/** Attribute Number */
#define _LLDPMAN_LLDPEN_ATTR_NUM        (1)
#define _LLDPMAN_MSGTXINV_ATTR_NUM      (2)
#define _LLDPMAN_MSGTXHOL_ATTR_NUM      (3)
#define _LLDPMAN_LLDPDB_ATTR_NUM        (4)
#define _LLDPMAN_LASTCHG_ATTR_NUM       (5)

/** Attribute value */
#define _LLDPMAN_LLDPEN_LEN             (OPENER_ETHLINK_INSTANCE_CNT + 1)
#define _LLDPMAN_DEF_MSGTXHOL_VAL       (4)  /* The recommended default value for msgTxHold is 4. */
#define _LLDPMAN_DEF_MSGTXINV_VAL       (30) /* The recommended default value for msgTxInterval is 30 seconds. */
#define _LLDPMAN_TXRX_ENABLED           (1)
#define _LLDPMAN_TXRX_DISABLED          (0)
#define _LLDPMAN_TX_ENABLED             (1)
#define _LLDPMAN_TX_DISABLED            (0)
#define _LLDPMAN_DATASTORE_LLDP         (0x1) /* Bit 0 = true, LLDP Data Table Object */
#define _LLDPMAN_DATASTORE_SNMP         (0x2) /* Bit 1 = true, SNMP */
#define _LLDPMAN_DATASTORE_NETCONF_YANG (0x4) /* Bit 2 = true, NETCONF YANG */

/** For LWIP LLDP adminStatus variables */
#define _LLDPMAN_ADMINSTATUS_TXRX_MASK   (0x3) /* bit mask to keep low 2 bit (TX, RX Enable) */
#define _LLDPMAN_ADMINSTATUS_TX_MASK     (0x1) /* bit mask to keep low 1 bit (TX Enable) */
...
#if defined (OPENER_TCPIP_IFACE_ACD_ENABLE) && \
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
static usr_err_t _set_cip_tcpip_network_information( opener_port_lwip_port_ctrl_t * const p_ctrl, lwip_port_netif_info_t * p_netif_info );
#endif /* ... && 0 != OPENER_TCPIP_IFACE_ACD_ENABLE */
static usr_err_t _open_cip_lldpmanagement_instance( opener_port_lwip_port_ctrl_t * const p_ctrl );
static usr_err_t _init_cip_lldpmanagement_instance_attributes( opener_port_lwip_port_ctrl_t * const p_ctrl );
static EipStatus _callback_lldpmanagement_pre_get( CipInstance *const p_instance,
		                                           CipAttributeStruct *const p_attribute,
												   CipByte service );
static EipStatus _callback_lldpmanagement_post_set( CipInstance *const p_instance,
		                                            CipAttributeStruct *const p_attribute,
													CipByte service );
static usr_err_t _get_lldp_array(uint8_t * p_lldp_array);
static usr_err_t _set_lldp_array(uint8_t * p_lldp_array);
                                         ...
                                         opener_port_cip_settable_nvdata_t const * const p_cip_nvdata_cfg)
{
    ...
    p_ctrl->nvdata_state = OPENER_PORT_NVDATA_STATE_NON;
    ...
    /** Initialize TCP/IP interface object instance */
    usr_err = _init_cip_tcpip_instance_attributes(p_ctrl);
    USR_ERROR_RETURN( USR_SUCCESS == usr_err, USR_ERR_NOT_INITIALIZED);

    /** Open LLDP Management object instance */
    usr_err = _open_cip_lldpmanagement_instance(p_ctrl);
    USR_ERROR_RETURN( USR_SUCCESS == usr_err, USR_ERR_NOT_INITIALIZED);

    /** Initialize LLDP Management object instance */
    usr_err = _init_cip_lldpmanagement_instance_attributes(p_ctrl);
    USR_ERROR_RETURN( USR_SUCCESS == usr_err, USR_ERR_NOT_INITIALIZED);
    ...
    /** Set network acd enable flag */
    /** Resolve type */
    CipTcpIpObject * p_tcpip_cip_instance = p_ctrl->p_tcpip_cip_instance;
    usr_err = p_ctrl->p_lwip_port_instance->p_api->netifAcdSet(p_ctrl->p_lwip_port_instance->p_ctrl, p_tcpip_cip_instance->select_acd);
...
/*******************************************************************************************************************//**
 * @brief return whether the lwIP network interface has a duplicate IP address
 *
 * @retval USR_SUCCESS                  The network interface has a duplicate IP address
 * @retval USR_NOT_ENABLE               The network interface does not have a duplicate IP address
 * @retval USR_ERR_ABORTED              Process has failed.
 **********************************************************************************************************************/
usr_err_t um_opener_port_lwip_port_netif_has_ip_duplicate( opener_port_lwip_port_ctrl_t * const p_ctrl )
{
    /** Error codes. */
    usr_err_t usr_err;

    /** Get lwIP network information */
    lwip_port_netif_info_t lwip_port_netif_info;

    /** Get lwIP network interface information. */
    usr_err = p_ctrl->p_lwip_port_instance->p_api->netifInfoGet(p_ctrl->p_lwip_port_instance->p_ctrl, &lwip_port_netif_info );
    USR_ERROR_RETURN( USR_SUCCESS == usr_err, USR_ERR_ABORTED );

    /** If netif does not have a duplicate IP address, return non-success code */
    if( lwip_port_netif_info.acd_fault == 0 )
    {
        return USR_ERR_NOT_ENABLED;
    }
...
#if defined(OPENER_CIP_SAFETY)
/*******************************************************************************************************************//**
 * @brief return whether the lwIP network interface recieves Proposed TUNID
 *
 * @retval USR_SUCCESS                  The network interface recieves Proposed TUNID
 * @retval USR_NOT_ENABLE               The network interface does not recieve Proposed TUNID
 **********************************************************************************************************************/
usr_err_t um_opener_port_lwip_port_netif_rcv_proposed_tunid( opener_port_lwip_port_ctrl_t * const p_ctrl )
{
    /** Error codes. */
    usr_err_t usr_err;

    /** If netif does not recieve Proposed TUNID, return non-success code */
    if( CipSafetySupervisorGetNETLEDflash() == false )
    {
        return USR_ERR_NOT_ENABLED;
    }
...
/*******************************************************************************************************************//**
 * @brief return whether lwIP network interface need to store TCP/IP settable nvdata
 *
 * @retval USR_SUCCESS                  The network interface need to save TCP/IP settable nvdata
 * @retval USR_NOT_ENABLE               The network interface does not no need to save TCP/IP settable nvdata
 **********************************************************************************************************************/
usr_err_t um_opener_port_lwip_port_state_need_store_nvdata_tcp( opener_port_lwip_port_ctrl_t * const p_ctrl )
{
    /** If the state is such that there is no need to save TCP/IP settable nvdata, return non-success code */
    if( p_ctrl->nvdata_state != OPENER_PORT_NVDATA_STATE_FOR_TCP )
    {
        return USR_ERR_NOT_ENABLED;
    }
...

{
  ...
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
    /** Get #10 select acd */
    p_cip_tcpip_cfg->select_acd = p_tcpip_cip_instance->select_acd;

    /** Get #11 last conflict detected */
    p_cip_tcpip_cfg->acd_activity = p_tcpip_cip_instance->last_conflict.acd_activity;
    memcpy(p_cip_tcpip_cfg->remote_mac, p_tcpip_cip_instance->last_conflict.remote_mac, sizeof(p_tcpip_cip_instance->last_conflict.remote_mac));
    memcpy(p_cip_tcpip_cfg->arp_pdu, p_tcpip_cip_instance->last_conflict.arp_pdu, sizeof(p_tcpip_cip_instance->last_conflict.arp_pdu));
#endif /* ... && 0 != OPENER_TCPIP_IFACE_ACD_ENABLE */
...
static void _lwip_port_callback_func( lwip_port_callback_args_t * p_arg )
{
  ...
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
    ...
    case LWIP_PORT_CALLBACK_EVENT_NETIF_UP:
        ...
        usr_err = _set_cip_tcpip_interface_configuration_status( p_ctrl, _TCPIP_STATUS_IFCFG_CONFIGURED_HARDWARE_SETTING );
        if( USR_SUCCESS != usr_err )
        {
            /** Error handling */
        }
...
#if defined (OPENER_TCPIP_IFACE_ACD_ENABLE) && \
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
    case LWIP_PORT_CALLBACK_EVENT_ACD:
        ...
        /** Set network interface information to CIP stack. */
        usr_err = _set_cip_tcpip_network_information( p_ctrl, &lwip_port_netif_info );
        if( USR_SUCCESS != usr_err )
        {
            /** Error handling */
        }
...
static usr_err_t _init_cip_tcpip_instance_attributes( opener_port_lwip_port_ctrl_t * const p_ctrl )
{
    ...
    p_tcpip_cip_instance->config_capability |= _TCPIP_CFGCAP_HARDWARE_CONFIGURABLE_BIT;
#if defined (OPENER_TCPIP_IFACE_ACD_ENABLE) && \
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
    p_tcpip_cip_instance->config_capability |= _TCPIP_CFGCAP_ACD_CAPABLE;
#endif /* ... && 0 != OPENER_TCPIP_IFACE_ACD_ENABLE */
    ...
    /** Set the host name (#6) */
    (void) SetCipStringByCstr( &p_tcpip_cip_instance->hostname, (char const *) p_ctrl->p_lwip_port_instance->p_cfg->p_netif_cfg->p_host_name );
...
static usr_err_t _set_cip_tcpip_network_configuration( opener_port_lwip_port_ctrl_t * const p_ctrl, lwip_port_netif_cfg_t * p_netif_cfg )
{
  ...
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
/*******************************************************************************************************************//**
 * @brief Set network interface information to TCP/IP object.
 *
 * @retval USR_SUCCESS      Process has been done successfully.
 **********************************************************************************************************************/
static usr_err_t _set_cip_tcpip_network_information( opener_port_lwip_port_ctrl_t * const p_ctrl, lwip_port_netif_info_t * p_netif_info )
{
    ...
    /** Set the status (#1) */
    if (p_netif_info->acd_status == 1)
    {
        /** Set target bits */
        p_tcpip_cip_instance->status |= _TCPIP_STATUS_IFCFG_ACD_STATUS_BIT;
    }
    else
    {
        /** Clear target bits */
    ...
    if (p_netif_info->acd_fault == 1)
    {
        /** Set target bits */
        p_tcpip_cip_instance->status |= _TCPIP_STATUS_IFCFG_ACD_FAULT_BIT;
    }
    else
    {
        /** Clear target bits */
    ...
    if (p_netif_info->acd_nvdata_flag == 1)
    {
        /** Set the LastConflictDetected (#11) */
        p_tcpip_cip_instance->last_conflict.acd_activity = p_netif_info->acd_activity;
        memcpy(p_tcpip_cip_instance->last_conflict.remote_mac, p_netif_info->remote_mac, sizeof(p_tcpip_cip_instance->last_conflict.remote_mac));
        memcpy(p_tcpip_cip_instance->last_conflict.arp_pdu, p_netif_info->arp_pdu, sizeof(p_tcpip_cip_instance->last_conflict.arp_pdu));

        /** Set state to store TCP/IP settable nvdata. */
        p_ctrl->nvdata_state = OPENER_PORT_NVDATA_STATE_FOR_TCP;
    }
...
/*******************************************************************************************************************//**
 * @brief Setup CIP LLDP Management instance with module controller.
 *
 * @param[in] p_ctrl                    Pointer to the controller
 * @retval USR_SUCCESS                  Process has been done successfully.
 **********************************************************************************************************************/
static usr_err_t _open_cip_lldpmanagement_instance( opener_port_lwip_port_ctrl_t * const p_ctrl )
{
	...
	p_ctrl->p_lldpmanagement_cip_instance = &g_lldpmanagement;
...
/*******************************************************************************************************************//**
 * @brief Initialize CIP LLDP Management attributes
 *
 * NOTE:
 *  This function initialize all attributes even if the attributes have already initialized on definition or on CipStackInit() function.
 *  This function should be called after the CIP stack initialization to overwrite attributes by the CIP initialization.
...
static usr_err_t _init_cip_lldpmanagement_instance_attributes( opener_port_lwip_port_ctrl_t * const p_ctrl )
{
    ...
    /** Resolve types */
    CipLLDPManagementObject * p_lldpmanagement_cip_instance;
    p_lldpmanagement_cip_instance = p_ctrl->p_lldpmanagement_cip_instance;
    uint8_t * p_lldp_array;
    p_lldp_array = &p_lldpmanagement_cip_instance->lldp_enable.array;

    /** Set attributes to initial values */
    p_lldpmanagement_cip_instance->lldp_enable.length = _LLDPMAN_LLDPEN_LEN;

    memset(p_lldp_array, 0, sizeof(uint8_t));
    *p_lldp_array = _LLDPMAN_TXRX_ENABLED;
    _get_lldp_array(p_lldp_array);

    p_lldpmanagement_cip_instance->msgTxInterval = _LLDPMAN_DEF_MSGTXINV_VAL;
    p_lldpmanagement_cip_instance->msgTxHold = _LLDPMAN_DEF_MSGTXHOL_VAL;
    p_lldpmanagement_cip_instance->lldp_datastore = _LLDPMAN_DATASTORE_SNMP;

    /** Set callback functions related attributes */
    CipClass * p_lldpmanagement_class = GetCipClass(kCipLLDPManagementClassCode);
    InsertGetSetCallback(p_lldpmanagement_class, _callback_lldpmanagement_pre_get, kPreGetFunc);
    InsertGetSetCallback(p_lldpmanagement_class, _callback_lldpmanagement_post_set, kPostSetFunc);
...
/*******************************************************************************************************************//**
 * @brief Callback function called before Get attribute service is replied
 *
 * * NOTE:
 *  This function is registered as callback function for LLDP Management Object.
 *  This is called before GetAttributeSingle processing.
 *
 ...
 **********************************************************************************************************************/
static EipStatus _callback_lldpmanagement_pre_get( CipInstance *const p_instance, CipAttributeStruct *const p_attribute, CipByte service )
{
	/** Unused parameter */
	(void) p_instance;
	...
	/** Check service and attribute */
	if ((service == kGetAttributeAll) || (service == kGetAttributeSingle))
	{
		switch(attr){
		case _LLDPMAN_LLDPEN_ATTR_NUM:
			/** Get and update LLDP Enable Status */
			_get_lldp_array(&g_lldpmanagement.lldp_enable.array);
			break;
		case _LLDPMAN_MSGTXINV_ATTR_NUM:
			/** Get and update msgTxInterval value */
			break;
		case _LLDPMAN_MSGTXHOL_ATTR_NUM:
			/** Get and update msgTxHold value */
			break;
		case _LLDPMAN_LLDPDB_ATTR_NUM:
			/** Do nothing */
			break;
		case _LLDPMAN_LASTCHG_ATTR_NUM:
			/** Get Last Change value */
			break;
		default:
...
/*******************************************************************************************************************//**
 * @brief Callback function called after Set attribute service is replied
 *
 * * NOTE:
 *  This function is registered as callback function for LLDP Management Object.
 *  This is called after SetAttributeSingle processing.
 *
 * @param[in] p_instance                Pointer of CIP instance data
 * @param p_attribute                   Pointer of CIP attribute data
 * @param[in] service                   Service code received from requester
 ...
 * @retval USR_ERROR                    Invalid Attribute number
 **********************************************************************************************************************/
static EipStatus _callback_lldpmanagement_post_set( CipInstance *const p_instance, CipAttributeStruct *const p_attribute, CipByte service )
{
	/** Unused parameter */
	(void) p_instance;
	...
	/** Check service and attribute */
	if (service == kSetAttributeSingle)
	{
		switch(attr){
		case _LLDPMAN_LLDPEN_ATTR_NUM:
			/** Set LLDP Enable Status */
			_set_lldp_array(&g_lldpmanagement.lldp_enable.array);
			break;
		case _LLDPMAN_MSGTXINV_ATTR_NUM:
			/** Set msgTxInterval value */
			break;
		case _LLDPMAN_MSGTXHOL_ATTR_NUM:
			/** Set msgTxHold value */
			break;
...
/*******************************************************************************************************************//**
 * @brief Get LLDP array status
 *
 * NOTE:
 *  This function gets LLDP array status from global variable of LWIP LLDP.
 *
 * @param[in] p_lldp_array              Pointer to the LLDP array
 * @retval USR_SUCCESS                  Process has been done successfully.
 * @retval USR_ERR_ASSERTION            Invalid value set in variable.
 **********************************************************************************************************************/
static usr_err_t _get_lldp_array(uint8_t * p_lldp_array)
{
	uint8_t mask;
	...
	for (uint8_t lldp_port_id = 1;  lldp_port_id <= OPENER_ETHLINK_INSTANCE_CNT; lldp_port_id++)
	{
		mask = (uint8_t) (1 << lldp_port_id);

		if ((LWIP_LLDP_get_g_adminStatus(lldp_port_id) == LWIP_PORT_LLDP_enabledRxTx)
			|| (LWIP_LLDP_get_g_adminStatus(lldp_port_id) == LWIP_PORT_LLDP_enabledTxOnly))
		{
			*p_lldp_array = (uint8_t)((*p_lldp_array & ~mask) | (_LLDPMAN_TX_ENABLED << lldp_port_id));
		}
		else if ((LWIP_LLDP_get_g_adminStatus(lldp_port_id) == LWIP_PORT_LLDP_enabledRxOnly)
				 || (LWIP_LLDP_get_g_adminStatus(lldp_port_id) == LWIP_PORT_LLDP_disabled))
		{
			*p_lldp_array = (uint8_t)((*p_lldp_array & ~mask) | (_LLDPMAN_TX_DISABLED << lldp_port_id));
		}
		else
		{
		    /** Return error code. */
...
/*******************************************************************************************************************//**
 * @brief Set LLDP array status
 *
 * NOTE:
 *  This function sets LLDP array status to global variable of LWIP LLDP.
 *
 * @param[in] p_lldp_array              Pointer to the LLDP array
 * @retval USR_SUCCESS                  Process has been done successfully.
 * @retval USR_ERR_ASSERTION            Invalid value set in variable.
...
static usr_err_t _set_lldp_array(uint8_t * p_lldp_array)
{
	...
	if ((*p_lldp_array & mask) == _LLDPMAN_TXRX_DISABLED)
	{
		for (uint8_t lldp_port_id = 1; lldp_port_id <= OPENER_ETHLINK_INSTANCE_CNT; lldp_port_id++)
		{
			LWIP_LLDP_set_g_adminStatus(lldp_port_id, LWIP_PORT_LLDP_disabled);
		}
	}
	else
	{
		for (uint8_t lldp_port_id = 1; lldp_port_id <= OPENER_ETHLINK_INSTANCE_CNT; lldp_port_id++)
		{
			/** Extract status of specified port */
			temp_val = *p_lldp_array;
			temp_val >>= lldp_port_id;
			temp_val &= mask;

			if (temp_val == _LLDPMAN_TX_ENABLED)
			{
				/** Get adminStatus */
				temp_val = LWIP_LLDP_get_g_adminStatus(lldp_port_id);
				/** Clear disable bit (bit 3) */
				temp_val &= _LLDPMAN_ADMINSTATUS_TXRX_MASK;
				/** Set TX Enable bit with keeping RX Enable bit */
				temp_val |= LWIP_PORT_LLDP_enabledTxOnly;
				LWIP_LLDP_set_g_adminStatus(lldp_port_id, (uint8_t)temp_val);
			}
			else
			{
				/** Set AdminStatus to TX disabled */
				switch (LWIP_LLDP_get_g_adminStatus(lldp_port_id))
				{
				case LWIP_PORT_LLDP_enabledTxOnly:
					LWIP_LLDP_set_g_adminStatus(lldp_port_id, LWIP_PORT_LLDP_disabled);
					break;
				case LWIP_PORT_LLDP_enabledRxOnly:
					/** Do nothing */
					break;
				case LWIP_PORT_LLDP_enabledRxTx:
					LWIP_LLDP_set_g_adminStatus(lldp_port_id, LWIP_PORT_LLDP_enabledRxOnly);
					break;
				case LWIP_PORT_LLDP_disabled:
					/** Do nothing */
					break;
				default:
					/** AdminStatus is invalid value */
					break;
				}
			}
		}
