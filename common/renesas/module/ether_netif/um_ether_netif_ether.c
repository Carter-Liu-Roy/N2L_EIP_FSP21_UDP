...
#include "r_gmac.h"
#endif
#if UM_ETHER_NETIF_FEATURE_R_ETHER_IMPL_R_GMAC_B
#include "r_gmac_b.h"
#endif
...
#if UM_ETHER_NETIF_FEATURE_R_ETHER_IMPL_R_GMAC || UM_ETHER_NETIF_FEATURE_R_ETHER_IMPL_R_GMAC_B
...
usr_err_t um_ether_netif_ether_monitor (ether_netif_ether_ctrl_t * const p_ctrl, uint32_t * p_link_status )
{
    ...
    *p_link_status &= ~ETHER_NETIF_PORT_ALL;
    ...
    for( port = 0; port < BSP_FEATURE_GMAC_MAX_PORTS; port++ )
    {
        ...
        {
#if defined(BSP_MCU_GROUP_RZT2H) || defined(BSP_MCU_GROUP_RZN2H)
            fsp_err = R_GMAC_B_GetLinkStatus(p_ctrl->p_ether_instance->p_ctrl, port, &port_status);
#else
        ...
        {
            if( GMAC_LINK_STATUS_READY == port_status || GMAC_LINK_STATUS_UP == port_status )
            {
