...
void EncodeSafetyNetworkNumber(const void *const data,
                               ENIPMessage *const outgoing_message) {
#if defined(OPENER_CIP_SAFETY)
  CipByteArray safety_net_num_array = {6, g_tcpip.safety_net_num};

  EncodeCipByteArray(&safety_net_num_array, outgoing_message);
#else
...
#if defined (OPENER_TCPIP_IFACE_ACD_ENABLE) && \
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
  CipByteArray remote_mac_byte_array = {6, g_tcpip.last_conflict.remote_mac};
  CipByteArray arp_pdu_byte_array = {28, g_tcpip.last_conflict.arp_pdu};

  EncodeCipUsint(&(g_tcpip.last_conflict.acd_activity), outgoing_message);
  EncodeCipByteArray(&remote_mac_byte_array, outgoing_message);
  EncodeCipByteArray(&arp_pdu_byte_array, outgoing_message);
#else
...
#if defined(OPENER_CIP_SAFETY)
static EipStatus TcpIpInterfacePreGetCallback(CipInstance *const instance,
  ...
  if (instance->instance_number == 1) {
    /* Instance 1 */
    if (((attribute->attribute_number == 7) && (service == kGetAttributeSingle)) || 
        ((attribute->attribute_number == 0) && (service == kGetAttributeAll))) {
      /* Safety Network Number */
      CipSafetySupervisorGetSafetyNetworkNumber(g_tcpip.safety_net_num);
    }
  }
...
EipStatus SetAttributeSingleTcpIpInterface(
  ...
  0 != OPENER_TCPIP_IFACE_ACD_ENABLE
        case 10: {
          CipBool select_acd = GetUsintFromMessage(
            &(message_router_request->data) );
          if ( select_acd > 0x01 ) {
            message_router_response->general_status =
              kCipErrorInvalidAttributeValue;
            ...
            if (attribute->data != NULL) {
              CipBool *data = (CipBool *) attribute->data;
              *(data) = select_acd;
              message_router_response->general_status = kCipErrorSuccess;
            } else {
              message_router_response->general_status = kCipErrorNotEnoughData;
        ...
        case 11: {
          CipUsint tmp_array[35] = {0};

          if (message_router_request->request_path_size < 35) {
            message_router_response->general_status =
              kCipErrorNotEnoughData;

          } else if (message_router_request->request_path_size > 35) {
            message_router_response->general_status =
              kCipErrorTooMuchData;

          } else if (memcmp(tmp_array, message_router_request->data, sizeof(tmp_array)) != 0) {
            message_router_response->general_status =
              kCipErrorInvalidAttributeValue;
            ...
            if (attribute->data != NULL) {
              memset(attribute->data, 0, sizeof(LastConflictDetected));
              message_router_response->general_status = kCipErrorSuccess;
            } else {
              message_router_response->general_status = kCipErrorNotEnoughData;
            }
...
EipStatus CipTcpIpInterfaceInit() {
  ...
  InsertAttribute(instance,
                  ...
                  &g_tcpip.safety_net_num[0],
                  kGetableSingleAndAll | kPreGetFunc);
#else
  InsertAttribute(instance,
                  7,
                  kCipAny,
  ...
  InsertAttribute(instance, 10, kCipBool, EncodeCipBool, &g_tcpip.select_acd,
                  kGetableSingleAndAll | kNvDataFunc | kSetable);
  InsertAttribute(instance,
                  11,
                  kCipBool,
                  EncodeCipLastConflictDetected,
                  &g_tcpip.last_conflict,
                  kGetableSingleAndAll | kNvDataFunc | kSetable);
#else
  InsertAttribute(instance, 10, kCipBool, EncodeCipBool, &g_tcpip.select_acd,
                  kGetableAllDummy);
  ...
  InsertService(tcp_ip_class, kSetAttributeSingle,
                ...
                "SetAttributeSingleTcpIpInterface");
#if defined(OPENER_CIP_SAFETY)
  InsertGetSetCallback(tcp_ip_class, TcpIpInterfacePreGetCallback, kPreGetFunc);
#endif
