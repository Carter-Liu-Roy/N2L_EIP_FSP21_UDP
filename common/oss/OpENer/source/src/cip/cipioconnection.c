...
#if defined(OPENER_CIP_SAFETY)
      if ( io_connection_object->is_safety == false ) {
...
#endif
#if defined(OPENER_CIP_SAFETY)
    if ( io_connection_object->is_safety == true ) {
      if ( ( (CipByteArray *) attribute->data )->length == 0 ) {
        data_size -= 6; /* remove the 6 bytes needed for Extended Time Coordination Format */
        diff_size += 6;
      } else if ( ( (CipByteArray *) attribute->data )->length == 1) {
        data_size -= 7; /* remove the 7 bytes needed for 1 or 2 Byte Data section  */
        diff_size += 7;
      } else if ( ( (CipByteArray *) attribute->data )->length == 2) {
        data_size -= 6; /* remove the 6 bytes needed for 1 or 2 Byte Data section  */
        diff_size += 6;
      } else {
        data_size -= (( (CipByteArray *) attribute->data )->length + 8); /* remove the length + 8 bytes needed for 3 to 250 Byte Data section */
        diff_size += (( (CipByteArray *) attribute->data )->length + 8);
      }
    }
#endif
  ...
  if ( NULL
    ...
    if ( io_connection_object->is_safety == true ) {
      if ( ( (CipByteArray *) attribute->data )->length == 0 ) {
        ...
        if ( kConnectionObjectConnectionTypeMulticast == ConnectionObjectGetTToOConnectionType(io_connection_object) ) {
          data_size -= (7 + 6); /* remove the 7 + 6 bytes needed for 1 or 2 Byte Data section & Extended Time Correction Format */
          diff_size += (7 + 6);
        } else {
          data_size -= 7; /* remove the 7 bytes needed for 1 or 2 Byte Data section  */
          diff_size += 7;
        ...
        if ( kConnectionObjectConnectionTypeMulticast == ConnectionObjectGetTToOConnectionType(io_connection_object) ) {
          data_size -= (6 + 6); /* remove the 6 + 6 bytes needed for 1 or 2 Byte Data section & Extended Time Correction Format */
          diff_size += (6 + 6);
        } else {
          data_size -= 6; /* remove the 6 bytes needed for 1 or 2 Byte Data section  */
          diff_size += 6;
        ...
        if ( kConnectionObjectConnectionTypeMulticast == ConnectionObjectGetTToOConnectionType(io_connection_object) ) {
          data_size -= ((( (CipByteArray *) attribute->data )->length + 8) + 6); /* remove the (length + 8) + 6 bytes needed for 3 to 250 Byte Data section & Extended Time Correction Format */
          diff_size += ((( (CipByteArray *) attribute->data )->length + 8) + 6);
        } else {
          data_size -= (( (CipByteArray *) attribute->data )->length + 8); /* remove the length + 8 bytes needed for 3 to 250 Byte Data section */
          diff_size += (( (CipByteArray *) attribute->data )->length + 8);
        }
...
#if 1 /* Renesas EIP Sample : Sequenced Address Item shall be used for CIP transport class 0 and class 1 connected data */
...
/* set AddressInfo Items to invalid Type */
  ...
  if (connection_object->is_safety == true) {
    if (producing_instance_attributes->length == 0) {
      /* Extended Time Coordination Format */
      common_packet_format_data->data_item.length += 6;
    } else if (producing_instance_attributes->length == 1) {
      /* 1 Byte */
      if (kConnectionObjectConnectionTypeMulticast == ConnectionObjectGetTToOConnectionType(connection_object)) {
        /* Extended Format 1-2 Byte Data Message & Extended Time Correction Format */
        common_packet_format_data->data_item.length += (7 + 6);
      } else {
        /* Extended Format 1-2 Byte Data Message */
        common_packet_format_data->data_item.length += 7;
      }
    } else if (producing_instance_attributes->length == 2) {
      /* 2 Byte */
      if (kConnectionObjectConnectionTypeMulticast == ConnectionObjectGetTToOConnectionType(connection_object)) {
        /* Extended Format 1-2 Byte Data Message & Extended Time Correction Format */
        common_packet_format_data->data_item.length += (6 + 6);
      } else {
        /* Extended Format 1-2 Byte Data Message */
        common_packet_format_data->data_item.length += 6;
      }
      ...
      if (kConnectionObjectConnectionTypeMulticast == ConnectionObjectGetTToOConnectionType(connection_object)) {
        /* Extended 3 to 250 byte Data Format & Extended Time Correction Format */
        common_packet_format_data->data_item.length += ((producing_instance_attributes->length + 8) + 6);
      } else {
        /* Extended 3 to 250 byte Data Format */
        common_packet_format_data->data_item.length += (producing_instance_attributes->length + 8);
      }
    }
  }
#endif
  ...
  if (connection_object->is_safety == true) {
    ...
    if (0 != cip_safety_imp_msg_wait(connection_object->cip_produced_connection_id, sizeof(rx_buf), &rx_len, rx_buf)) {
      return kEipStatusError;
    }
    memcpy(outgoing_message.current_message_position, &rx_buf[0], rx_len);

    outgoing_message.current_message_position +=
      common_packet_format_data->data_item.length;
    outgoing_message.used_message_length += common_packet_format_data->data_item.length;
  } else {
...
#if defined(OPENER_CIP_SAFETY)
  if(connection_object->is_safety == true) {
    if(0 != cip_safety_imp_msg_send(connection_object->cip_consumed_connection_id, data_length, data)) {
      return kEipStatusError;
    }
    return kEipStatusOk;
  }
