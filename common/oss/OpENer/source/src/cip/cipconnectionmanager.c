...
EipUint32 g_connection_id_initial_offset; /* This variable is workaround for the initial number of Connection ID */
...
CipUdint GetConnectionId(void) {
  ...
  connection_id++;
  connection_id += g_connection_id_initial_offset; /* This variable is workaround for the Connection ID duplication error */
                                                   /* In a future update, this will be removed. */
#if defined(OPENER_CIP_SAFETY)
  /* Standard IO uses 0x00000000 to 0x7FFFFFFF */
  return ((g_incarnation_id | (connection_id & 0x0000FFFF)) & 0x7FFFFFFF);
#else
...
EipStatus HandleNonNullNonMatchingForwardOpenRequest(
  ...
  CipOctet app_reply[14] = {0};
  CipUdint cip_produced_connection_id = 0;
  CipUdint cip_consumed_connection_id = 0;

  if (g_dummy_connection_object.is_safety == true) {
    message_router_response->reply_service = (0x80 | message_router_request->service);
    if (0 != cip_safety_exp_msg_wait(sizeof(rx_buf), &rx_len, rx_buf, message_router_response)) {
        temp = kCipErrorResourceUnavailable;
    } else {
      if (message_router_response->general_status == kCipErrorSuccess) {
        temp = kEipStatusOk;
        connection_status = kConnectionManagerExtendedStatusCodeSuccess;
        memcpy(&cip_consumed_connection_id, &rx_buf[0], sizeof(cip_consumed_connection_id));
        memcpy(&cip_produced_connection_id, &rx_buf[4], sizeof(cip_produced_connection_id));
        memcpy(&app_reply[0], &rx_buf[26], sizeof(app_reply));
      } else {
        return AssembleForwardOpenResponse(&g_dummy_connection_object,
                                           message_router_response,
  ...
  if(kEipStatusOk != temp) {
    ...
    if (g_dummy_connection_object.is_safety == true) {
      ...
      CipSafetySupervisorProcessSafetyOpen();
      /* Safety Validator Object */
      if (kConnectionObjectTransportClassTriggerDirectionClient ==
          ConnectionObjectGetTransportClassTriggerDirection(&g_dummy_connection_object)) {
        /* Producer */
        /* Sets Connection_ID for Implicit Message frame queue */
        cip_safety_imp_msg_set_connection_id(0, cip_produced_connection_id);
        /* Creates Safety Validator Instance */
        CreateSafetyValidatorObject(g_dummy_connection_object.produced_path.instance_id);
      } else {
        /* Consumer */
        /* Sets Connection_ID for Implicit Message frame queue */
        cip_safety_imp_msg_set_connection_id(1, cip_produced_connection_id);
        /* Creates Safety Validator Instance */
        CreateSafetyValidatorObject(g_dummy_connection_object.consumed_path.instance_id);
      }
      ConnectionObjectSetCipConsumedConnectionID(connection_list.first->data,
                                                 cip_consumed_connection_id);
      ConnectionObjectSetCipProducedConnectionID(connection_list.first->data,
  ...
  while(NULL != node) {
    ...
    if( (kConnectionObjectStateEstablished ==
      ...
      if( (connection_object->connection_serial_number ==
        ...
        if (connection_object->is_safety == true) {
          message_router_response->reply_service = (0x80 | message_router_request->service);
          if (0 != cip_safety_exp_msg_wait(sizeof(rx_buf), &rx_len, rx_buf, message_router_response)) {
            ...
            if (message_router_response->general_status == kCipErrorSuccess) {
              ...
              if (kConnectionObjectTransportClassTriggerDirectionClient ==
                ...
                /* Producer */
                /* Delete Safety Validator Instance */
                DeleteSafetyValidatorInstance(connection_object->produced_path.instance_id);
              } else {
                /* Consumer */
                /* Delete Safety Validator Instance */
                DeleteSafetyValidatorInstance(connection_object->consumed_path.instance_id);
              }
            } else {
              /* TODO: Specific Error */
              connection_status = kConnectionManagerExtendedStatusWrongCloser;
...
#if defined(OPENER_CIP_SAFETY)
      case kCipErrorResourceUnavailable:
#endif
...
#if defined(OPENER_CIP_SAFETY)
  if(app_reply != NULL) {
    AddSintToMessage(app_reply_size, &message_router_response->message); /* remaining path size - for routing devices relevant */
    AddSintToMessage(0, &message_router_response->message); /* reserved */
    for(size_t i = 0; i < (app_reply_size * 2); i++) {
      AddSintToMessage(app_reply[i], &message_router_response->message);
    }
  } else {
...
#if defined(OPENER_CIP_SAFETY)
        if(true == ConnectionObjectGetIsSafety(iterator->data) ) {
          if(kConnectionObjectTransportClassTriggerDirectionClient ==
             ConnectionObjectGetTransportClassTriggerDirection(iterator->data) ) {
            if(kConnectionObjectConnectionTypeMulticast ==
               ConnectionObjectGetTToOConnectionType(iterator->data) ) {
              CipUsint timeout_multiplier = ConnectionObjectGetConnectionTimeoutMultiplier(iterator->data);
              CipUsint timeout_multiplier_pi = 0;
              if (timeout_multiplier > 4) {
                timeout_multiplier_pi = 4;
              } else {
                timeout_multiplier_pi = timeout_multiplier;
              }
              CipUdint o_to_t_requested_packet_interval = ConnectionObjectGetOToTRequestedPacketInterval(iterator->data);
              uint64_t multi_cast_time_coordination_message_timeout_value = ( ( (uint64_t)(o_to_t_requested_packet_interval) /
                                                                              (uint64_t) 1000 ) * (timeout_multiplier_pi + 2) );
              uint64_t inactivity_watchdog_timer = ConnectionObjectGetInactivityWatchdogTimerValue(iterator->data);
              /* FRS98 */
              if (multi_cast_time_coordination_message_timeout_value < inactivity_watchdog_timer) {
                CloseCommunicationChannelsAndRemoveFromActiveConnectionsList(iterator->data);
                return NULL;
              }
            }
          }
...
#if defined(OPENER_CIP_SAFETY)
  connection_object->is_safety = false;
#endif
          ...
          case kSegmentTypeNetworkSegment: {
            ...
            switch(subtype) {
              case kNetworkSegmentSubtypeProductionInhibitTimeInMilliseconds:
                ...
                break;
#if defined(OPENER_CIP_SAFETY)
              case kNetworkSegmentSubtypeSafetySegment:
                connection_object->is_safety = true;
                CipBool config_data_in = (g_config_data_length > 0) ? true : false;
                CipBool output_device = (kConnectionObjectTransportClassTriggerDirectionServer == 
                                         ConnectionObjectGetTransportClassTriggerDirection(connection_object)) ? true : false;
                size_t safety_network_segment_length =
                  CipSafetySupervisorProcessSafetyNetworkSegment(&message[1], config_data_in, output_device, connection_object->consumed_path.instance_id);
                message += (safety_network_segment_length + 2);
                remaining_path -= (safety_network_segment_length + 2) / 2;
                break;
#endif
              default:
                OPENER_TRACE_ERR("Not allowed in connection manager");
  ...
  CloseSessionFunction CloseSessions)
{
  ...
  while(NULL != search_node) {
    ...
    if(kConnectionObjectConnectionTypeMulticast ==
       ConnectionObjectGetTToOConnectionType(connection_object) ) {
      if((connection_object->cip_produced_connection_id ==
         search_connection->cip_produced_connection_id) &&
...
#if defined(OPENER_CIP_SAFETY)
CipUdint CountEstablishedIOconnections()
{
  CipUdint count = 0;
  ...
  while(NULL != search_node) {
    CipConnectionObject *search_connection = search_node->data;
    if((((search_connection->instance_type == kConnectionObjectInstanceTypeIO) ||
         (search_connection->instance_type == kConnectionObjectInstanceTypeIOExclusiveOwner)) ||
         (search_connection->instance_type == kConnectionObjectInstanceTypeIOInputOnly)) ||
         (search_connection->instance_type == kConnectionObjectInstanceTypeIOListenOnly)) {
      count++;
    }
    search_node = search_node->next;
  }
