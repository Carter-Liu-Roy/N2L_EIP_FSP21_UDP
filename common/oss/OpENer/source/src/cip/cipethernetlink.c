...
#define COUNTER_NUMBER_OF_INTERFACE_COUNTERS_11 11
#define COUNTER_NUMBER_OF_MEDIA_COUNTERS_12     12
  ...
  if (ethernet_link_class != NULL) {
    ...
    for (size_t idx = 0; idx < OPENER_ETHLINK_INSTANCE_CNT; ++idx) {
      ...
      InsertAttribute(ethernet_link_instance,
                      ...
                      kGetableSingleAndAll|kPreGetFunc|kPostGetFunc);
      InsertAttribute(ethernet_link_instance,
                      5,
					  kCipAny,
...
{
#if defined(OPENER_ETHLINK_CNTRS_ENABLE) && 0 != OPENER_ETHLINK_CNTRS_ENABLE
	CipEthernetLinkInterfaceCounters * p_data = (CipEthernetLinkInterfaceCounters * ) data;

	memcpy(outgoing_message->current_message_position, p_data, COUNTER_NUMBER_OF_INTERFACE_COUNTERS_11 * sizeof(CipUdint));
	MoveMessageNOctets(COUNTER_NUMBER_OF_INTERFACE_COUNTERS_11 * sizeof(CipUdint), outgoing_message);
                                               ...
                                               ENIPMessage *const outgoing_message)
{
	...
	/* Encode the Media Counters data in the global variable to the message. */
	memcpy(outgoing_message->current_message_position, p_data, COUNTER_NUMBER_OF_MEDIA_COUNTERS_12 * sizeof(CipUdint));
	MoveMessageNOctets(COUNTER_NUMBER_OF_MEDIA_COUNTERS_12 * sizeof(CipUdint), outgoing_message);
