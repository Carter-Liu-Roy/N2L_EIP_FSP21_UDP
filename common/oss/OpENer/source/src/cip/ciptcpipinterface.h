...
/** @brief Last Conflict Detected struct, called LastConflictDetected
 *
 */
typedef struct last_conflict_detected {
  CipUsint acd_activity; /**< The ACD contains the state of the ACD algorithm when the last IP address conflict was detected. */
  CipUsint remote_mac[6]; /**< The IEEE 802.3 source MAC address from the header of the received Ethernet packet which was sent by a device reporting a conflict. */
  CipUsint arp_pdu[28]; /**< The ARP Response PDU in binary format. */
...
typedef struct {
  ...
  CipUsint safety_net_num[6]; /**< #6 safety network number */
#endif
  ...
  LastConflictDetected last_conflict;
