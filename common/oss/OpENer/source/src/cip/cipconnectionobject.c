...
uint64_t ConnectionObjectCalculateSafetyInactivityWatchdogTimerValue(
  const CipConnectionObject *const connection_object);
#endif
...
#if defined(OPENER_CIP_SAFETY)
  uint64_t calculated_timeout_value = 0;
  if(connection_object->is_safety == true) {
    calculated_timeout_value =
      ConnectionObjectCalculateSafetyInactivityWatchdogTimerValue(
        connection_object);
  } else {
    calculated_timeout_value =
...
#if defined(OPENER_CIP_SAFETY)
uint64_t ConnectionObjectGetInactivityWatchdogTimerValue(
  const CipConnectionObject *const connection_object) {
  return connection_object->inactivity_watchdog_timer;
}
#endif
...
uint64_t ConnectionObjectCalculateSafetyInactivityWatchdogTimerValue(
  ...
  if( kConnectionObjectTransportClassTriggerDirectionServer ==
    ...
    return ( ( (uint64_t)(connection_object->t_to_o_requested_packet_interval) /
               (uint64_t) 1000 ) *
             ((connection_object->connection_timeout_multiplier + 1) * 4) );
  } else {
    return ( ( (uint64_t)(connection_object->o_to_t_requested_packet_interval) /
               (uint64_t) 1000 ) *
             ((connection_object->connection_timeout_multiplier + 1) * 4) );
...
#if defined(OPENER_CIP_SAFETY)
CipBool ConnectionObjectGetIsSafety(
  const CipConnectionObject *const connection_object) {
  return connection_object->is_safety;
}
#endif
