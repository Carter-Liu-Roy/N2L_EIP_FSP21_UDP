...
void EncodeEPath(CipEpath *epath,
                 ENIPMessage *message);
#if defined(OPENER_CIP_SAFETY)
void EncodePackedEPath(CipEpath *epath,
                       ENIPMessage *message);
#endif
...
EipStatus CipStackInit(const EipUint16 unique_connection_id) {
  ...
  OPENER_ASSERT(kEipStatusOk == eip_status);
#endif
  eip_status = CipLLDPManagementInit();
  OPENER_ASSERT(kEipStatusOk == eip_status);
  eip_status = CipQoSInit();
  OPENER_ASSERT(kEipStatusOk == eip_status);
#if defined(OPENER_CIP_SAFETY)
  eip_status = CipSafetySupervisorInit();
  OPENER_ASSERT(kEipStatusOk == eip_status);
  eip_status = CipSafetyValidatorInit();
  OPENER_ASSERT(kEipStatusOk == eip_status);
  eip_status = CipSafetyAnalogInputPointInit();
  OPENER_ASSERT(kEipStatusOk == eip_status);
  eip_status = CipSafetyAnalogOutputPointInit();
  OPENER_ASSERT(kEipStatusOk == eip_status);
#endif
...
#if defined(OPENER_CIP_SAFETY)
void EncodeCipPackedEPath(const CipEpath *const data,
                          ENIPMessage *const outgoing_message) {
  EncodePackedEPath( (CipEpath *) data, outgoing_message );
}
#endif
...
EipStatus GetAttributeAll(CipInstance *instance,
  ...
  if(0 == instance->cip_class->number_of_attributes) {
	...
	/* Call the PreGetCallback if enabled for either attribute and the class provides one. */
	EipUint8 attribute_flag = 0;
	...
	if (attribute_flag) {
		instance->cip_class->PreGetCallback(instance, attribute,
				message_router_request->service);
		attribute_flag = 0;
	}

	/* Make response message for Get Attribute All */
	attribute = (CipAttributeStruct *) instance->attributes; /* pointer to list of attributes*/
    GenerateGetAttributeSingleHeader(message_router_request,
                                     message_router_response);
    message_router_response->general_status = kCipErrorSuccess;
    for(size_t j = 0; j < instance->cip_class->number_of_attributes; j++) {
	...
	/* Call the PostGetCallback if enabled for either attribute and the class provides one. */
	attribute = (CipAttributeStruct *) instance->attributes; /* pointer to list of attributes*/
	for (size_t j = 0; j < instance->cip_class->number_of_attributes; j++) {
		if ((attribute->attribute_flags & kPostGetFunc) &&
		NULL != instance->cip_class->PostGetCallback) {
	...
	if (attribute_flag) {
		instance->cip_class->PostGetCallback(instance, attribute,
				message_router_request->service);
		attribute_flag = 0;
	}
...
void EncodePackedEPath(CipEpath *epath,
                       ENIPMessage *message) {
  unsigned char length = (unsigned char)epath->path_size;
  size_t start_length = message->used_message_length;
  AddSintToMessage((EipUint8) epath->path_size, message);
  ...
  OPENER_ASSERT(
    1 + epath->path_size == message->used_message_length - start_length);             /* path size is in 8 bit chunks according to the specification */
}
#endif
